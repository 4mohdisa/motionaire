// Dev-remote test cases (debug builds only). This module is reached ONLY
// through an `import.meta.env.DEV`-guarded dynamic import in menuBridge —
// production builds contain none of it (the guard is statically false, so
// the chunk is never emitted). The cases themselves are the e2e suite:
// each prints exactly one WEBVIEW-TEST PASS/FAIL line via report_test.
import { invoke } from '@tauri-apps/api/core'
import { useStore } from '../state/store'
import { createProject } from '../types/project'
import { loadPipDemo } from '../compositor/bridge'
import { dispatch } from './menuBridge'
...
function ensureFx(clipId: string, type: import('../types/project').EffectType): string {
...
export async function handleDevCase(action: string): Promise<void> {
  ...
  switch (action) {
    ...
    case 'dev:p5_select_test': {
      ...
      try {
        ...
        const rejected = find(leftPiece.id).start === leftPiece.start && find(bId).start === bStart1
        void report(
    ...
    case 'dev:p4_text_test': {
      ...
      try {
        ...
        const overlayHidden = st().compositorActive && !document.querySelector('.preview__text')
        void report(
          overlayHidden,
        ...
        const still = st()
          ...
          .find(
            (c) =>
              c.mediaId &&
              st()
                .project.media.find((m) => m.id === c.mediaId)
    ...
    case 'dev:p6_safety_test': {
      ...
      try {
        ...
        const pasteKeepsState =
          !!pasted && pasted.effects.find((e) => e.type === 'grade')?.params.saturation === -0.4
        const before = st().project.tracks.flatMap((t) => t.clips).length
    ...
    case 'dev:f0_ctx_test': {
      ...
      try {
        ...
        void report(
          inView,
          `rect=${Math.round(r.left)},${Math.round(r.top)},${Math.round(r.right)},${Math.round(r.bottom)} vp=${window.innerWidth}x${window.innerHeight}`,
        )
      } catch (e) {
        void report(false, String(e))
      }
    ...
    case 'dev:f1_parity_test': {
      ...
      try {
        ...
        void report(
          worst < 2e-3,
          `worst=${worst.toExponential(2)} at ${worstAt}; samples=${samples.length}`,
        )
      } catch (e) {
        void report(false, String(e))
      }
    ...
    case 'dev:f3_audio_test': {
      ...
      try {
        ...
        let peak = 0
        for (
          let i = Math.floor(tc.in * wf.pps);
          i < Math.min(wf.peaks.length, tc.out * wf.pps);
          i++
        )
    ...
    case 'dev:f5_proxy_test': {
      ...
      try {
        ...
        const expPaths = flatten(st().project, { originals: true }).layers.map((l) => l.mediaPath)
        const ruleOk = prevPaths.every((p) => p === proxyPath) && expPaths.every((p) => p === uhd)
    ...
    case 'dev:p4_trim_test': {
      ...
      try {
        ...
        const rippleOk =
          Math.abs(cEnd(afterRipple[1]) - 5) < 0.15 && Math.abs(afterRipple[2].start - 5) < 0.15
    ...
    case 'dev:p8_org_test': {
      ...
      try {
        ...
        const pass =
          labelOk && selByLabel && tcOk && folderOk && liveCompound && looseAgain && chaptersOk
        void report(
          pass,
    ...
    case 'dev:f8_restore_test': {
      ...
      try {
        ...
        await wait(400)
        const btn = document.querySelector<HTMLButtonElement>('.launcher__recovery .shell__primary')
        if (!btn) {
    ...
    case 'dev:r1p3_tools_test': {
      ...
      try {
        ...
        const cutOk =
          t2.edited &&
          !t2.error &&
          Math.abs(durBefore - durAfter - 2) < 0.2 &&
    ...
    case 'dev:r1p4_chat_test': {
      ...
      try {
        ...
        example.click()
        const doneStreaming = await until(() => !st().chatBusy && st().chatLog.length >= 2, 20000)
        await wait(300)
    ...
    case 'dev:r1p6_gen_test': {
      ...
      try {
        ...
        const landed = await until(() => st().project.media.length === binBefore + 1, 60000)
        ...
        const placedClip = st()
          .project.tracks.flatMap((t) => t.clips)
          .find(
            (c) => st().project.media.find((m) => m.id === c.mediaId)?.aiGenerated && c.start < 0.5,
          )
        const pass = landed && flagged && probed && toastAction && toolOk && placed && !!placedClip
        void report(
          pass,
    ...
    case 'dev:r1p7_ready_test': {
      ...
      try {
        ...
        void report(
          pass,
          `editor=${inEditor} clips=${clips.length} kf=${kf} title=${hasTitle} compositor=${live}`,
        )
      } catch (e) {
        void report(false, `threw: ${e}`)
      }
    ...
    case 'dev:rel_export_test': {
      ...
      try {
        ...
        const t0 = Date.now()
        while (dones < 2 && Date.now() - t0 < 120000) await new Promise((r) => setTimeout(r, 300))
        unq()
        und()
    ...
    case 'dev:smoke': {
      ...
      try {
        ...
        const fpsLive = await until(() => st().compositorActive && st().compositorFps > 5, 4000)
        st().pause()
        ...
        const pass = placed >= 2 && advanced && fpsLive && hasText && exportOk && durOk
        void report(
          pass,
    ...
    case 'dev:f8_test': {
      ...
      try {
        ...
        const onDst = p()
          .tracks.find((t) => t.id === dstTrack.id)!
          .clips.map((c) => c.id)
